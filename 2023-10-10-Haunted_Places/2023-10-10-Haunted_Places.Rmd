...
```{r}
    place <- glue::glue('Mentryville, California')
    place <- stringr::str_replace_all(place, ' ', '_')
    # search
    res <- fromJSON(glue::glue("http://en.wikipedia.org/w/api.php?action=query&prop=pageimages&format=json&",
                             "piprop=original&list=search&srsearch={place}"))
...
### write out data
########################
# write_sf(haunted_places_county, "./output_data/haunted_places_county.shp")
# 
...
out <- get_image_url('Taft High School Woodland Hills, California')
